  ...
  return (
    <div style={{
      ...
      <div style={{ maxWidth: 1400, margin: "0 auto", padding: "36px 32px" }}>
        ...
        {/* ─── HEADER ─── */}
        <div style={{ marginBottom: 36, display: "flex", alignItems: "flex-end", gap: 20, flexWrap: "wrap" }}>
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 8 }}>
              <span style={{ fontSize: 44 }}>🍔</span>
              <h1 style={{
                margin: 0, fontSize: 40, fontWeight: 900, letterSpacing: "-1.5px",
                color: C.flame,
            ...
            <p style={{ margin: 0, fontSize: 15, color: C.textMuted, maxWidth: 680, lineHeight: 1.6 }}>
        ...
        <div style={{
          ...
          background: C.surface, padding: 5, borderRadius: 12,
            ...
            <div style={{ display: "flex", gap: 24, flexWrap: "wrap" }}>
              ...
              <div style={{ flex: "1 1 700px" }}>
                ...
                <p style={{ fontSize: 12, color: C.textMuted, textAlign: "center", margin: "10px 0 0" }}>
                  Click any service node to inspect its endpoints, communication pattern, and what it fixes from MrBeast Burger
                </p>
              </div>
              ...
              {svc ? (
                <div style={{
                  flex: "0 0 360px", background: C.surface,
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
                    ...
                    <h3 style={{ margin: 0, fontSize: 17, fontWeight: 800, color: svc.color }}>{svc.name}</h3>
                  </div>
                  <p style={{ fontSize: 12, color: C.textMuted, margin: "0 0 14px" }}>{svc.sub}</p>
                  <div style={{ display: "flex", gap: 8, marginBottom: 16, flexWrap: "wrap" }}>
                    <span style={{ fontSize: 11, padding: "4px 10px", background: svc.glow, border: `1px solid ${svc.color}50`, borderRadius: 6, color: svc.color }}>{svc.lang}</span>
                    <span style={{ fontSize: 11, padding: "4px 10px", background: C.greenGlow, border: `1px solid ${C.green}50`, borderRadius: 6, color: C.green }}>{svc.store}</span>
                  </div>
                  <p style={{ fontSize: 13, color: C.textSoft, lineHeight: 1.7, marginBottom: 18 }}>{svc.desc}</p>
                  <h4 style={{ fontSize: 11, color: C.textMuted, textTransform: "uppercase", letterSpacing: 1.5, marginBottom: 10, fontWeight: 800 }}>Endpoints / Channels</h4>
                  {svc.endpoints.map((ep, i) => (
                    <div key={i} style={{
                      fontSize: 12, padding: "6px 10px", background: C.bg,
                      borderRadius: 5, marginBottom: 4, color: C.flame, fontFamily: "monospace",
                    }}>{ep}</div>
                  ))}
                  <div style={{
                    ...
                    <h4 style={{ fontSize: 11, color: C.flame, textTransform: "uppercase", letterSpacing: 1.5, margin: "0 0 8px", fontWeight: 800 }}>
                      🔥 What This Fixes
                    </h4>
                    <p style={{ fontSize: 12, color: C.textSoft, lineHeight: 1.65, margin: 0 }}>{svc.fix}</p>
                  </div>
                </div>
              ) : (
                ...
                <div style={{
                  flex: "0 0 360px", background: C.surface,
                  border: `1px dashed ${C.border}`, borderRadius: 14, padding: 32,
                  ...
                  <p style={{ fontSize: 13, color: C.textMuted, margin: 0, lineHeight: 1.6 }}>
                    Click any service node on the diagram to see its endpoints, tech stack, and what problem it solves.
                  </p>
                </div>
              )}
            </div>
            ...
            <div style={{ maxWidth: 860 }}>
              <div style={{ display: "flex", gap: 10, marginBottom: 24 }}>
                <button onClick={() => { setShowRollback(false); setFlowStep(-1); }}
                  style={{
                    padding: "10px 22px", fontSize: 13, fontFamily: "inherit", fontWeight: 700,
                    border: `1px solid ${!showRollback ? C.green : C.border}`, borderRadius: 9, cursor: "pointer",
                ...
                <button onClick={() => { setShowRollback(true); setFlowStep(-1); }}
                  style={{
                    padding: "10px 22px", fontSize: 13, fontFamily: "inherit", fontWeight: 700,
                    border: `1px solid ${showRollback ? C.red : C.border}`, borderRadius: 9, cursor: "pointer",
              ...
              <div style={{
                ...
                <h3 style={{ margin: "0 0 20px", fontSize: 17, color: showRollback ? C.red : C.green, fontWeight: 800 }}>
                  ...
                  return (
                    <div key={i} className="flow-step" onClick={() => setFlowStep(i)}
                      style={{
                        ...
                        padding: "12px 16px", marginBottom: 4, borderRadius: 9,
                      ...
                      <span style={{
                        ...
                        background: C.bg, color: C.textMuted, width: 84, textAlign: "center",
                      ...
                      <span style={{ fontSize: 13, color: isActive ? C.text : C.textMuted }}>{step.label}</span>
                    </div>
                  );
                })}
                ...
                <div style={{ display: "flex", gap: 10, marginTop: 22 }}>
                  <button onClick={() => setFlowStep(f => Math.max(-1, f - 1))}
                    style={{ padding: "9px 22px", fontSize: 12, fontFamily: "inherit", border: `1px solid ${C.border}`, borderRadius: 7, cursor: "pointer", background: C.card, color: C.textMuted }}>
                    ← Prev
                  </button>
                  <button onClick={() => { const max = showRollback ? orderFlow.slice(0, 11).length + rollbackFlow.length - 1 : orderFlow.length - 1; setFlowStep(f => Math.min(max, f + 1)); }}
                    style={{ padding: "9px 22px", fontSize: 12, fontFamily: "inherit", border: `1px solid ${C.flame}`, borderRadius: 7, cursor: "pointer", background: C.flameGlow, color: C.flame }}>
                    Next →
                  </button>
  ...
  redis:
    ...
    ports: ["6379:6379"]`,
                ...
                <div key={i} className="card-hover" style={{
                  ...
                  padding: 24, borderTop: `3px solid ${item.color}`,
                }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
                    <span style={{ fontSize: 24 }}>{item.icon}</span>
                    <h3 style={{ margin: 0, fontSize: 15, fontWeight: 800, color: item.color }}>{item.title}</h3>
                  </div>
                  <span style={{
                    fontSize: 11, padding: "3px 10px", borderRadius: 5,
                    background: `${item.color}15`, border: `1px solid ${item.color}40`,
                    color: item.color, fontWeight: 700, display: "inline-block", marginBottom: 12,
                  }}>{item.target}</span>
                  <p style={{ fontSize: 13, color: C.textSoft, lineHeight: 1.7, marginBottom: 16 }}>{item.desc}</p>
                  <pre style={{
                    fontSize: 11, background: C.bg, border: `1px solid ${C.border}`,
                    borderRadius: 9, padding: 14, overflow: "auto", color: C.textMuted,
                    lineHeight: 1.6, margin: 0, maxHeight: 260,
                  }}>{item.yaml}</pre>
                </div>
              ))}
            </div>
          ...
          {tab === "failures" && (
            <div style={{ maxWidth: 900 }}>
              <div style={{
                ...
                <p style={{ fontSize: 14, color: C.textSoft, lineHeight: 1.75, margin: 0 }}>
                  ...
                  <div style={{ display: "flex", alignItems: "flex-start", gap: 16 }}>
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 10, flexWrap: "wrap" }}>
                        <h4 style={{ margin: 0, fontSize: 15, fontWeight: 800, color: C.text }}>{f.problem}</h4>
                        <span style={{
                          fontSize: 10, padding: "3px 10px", borderRadius: 5, fontWeight: 800,
                      ...
                      <div style={{
                        display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginTop: 10,
                      }}>
                        <div style={{ padding: "12px 14px", borderRadius: 9, background: C.redGlow, border: `1px solid ${C.red}20` }}>
                          <div style={{ fontSize: 10, fontWeight: 800, color: C.red, textTransform: "uppercase", letterSpacing: 1, marginBottom: 6 }}>❌ What happened</div>
                          <p style={{ fontSize: 12, color: C.textSoft, lineHeight: 1.6, margin: 0 }}>{f.what}</p>
                        </div>
                        <div style={{ padding: "12px 14px", borderRadius: 9, background: C.greenGlow, border: `1px solid ${C.green}20` }}>
                          <div style={{ fontSize: 10, fontWeight: 800, color: C.green, textTransform: "uppercase", letterSpacing: 1, marginBottom: 6 }}>✓ How v2.0 fixes it</div>
                          <p style={{ fontSize: 12, color: C.textSoft, lineHeight: 1.6, margin: 0 }}>{f.fix}</p>
                        </div>
                      </div>
                    </div>
                  </div>
          ...
          {tab === "stack" && (
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(320px, 1fr))", gap: 20 }}>
              {[
                ...
                <div key={i} className="card-hover" style={{
                  background: C.surface, border: `1px solid ${C.border}`, borderRadius: 14,
                  padding: 24, borderLeft: `3px solid ${g.color}`,
                }}>
                  <h3 style={{ margin: "0 0 16px", fontSize: 15, fontWeight: 800, color: g.color }}>{g.cat}</h3>
                  {g.items.map((item, j) => (
                    <div key={j} style={{
                      ...
                      <div>
                        ...
                        <div style={{ fontSize: 11, color: C.textMuted, marginTop: 3 }}>{item.note}</div>
                      </div>
                      <span style={{
                        fontSize: 11, padding: "4px 10px", background: C.bg, borderRadius: 5,
                        color: g.color, fontWeight: 700, whiteSpace: "nowrap", flexShrink: 0, marginLeft: 10,
                      }}>{item.tech}</span>
                    </div>
                  ))}
                </div>
